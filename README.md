...
**Способ 1 — прямо в роутере** (KeeneticOS 5.1.1+, можно удалённо): сначала в **Параметры системы → Изменить набор компонентов** установите компонент **«Утилиты для файловой системы ext»** (поиск: `утили`). Затем вставьте флешку → **Накопители и устройства** → напротив раздела **⋮ → Форматировать** → файловая система **EXT4** → Подтвердить. Флешку в компьютер вставлять не нужно.
